...
public class Line implements Shape {
	...
	public Line(Point p1, Point p2) {
		this.p1 = p1;
		this.p2 = p2;
		this.equation = LineEquations.create(p1.getX(), p1.getY(), p2.getX(),
				p2.getY());
	}
	...
	public boolean interescts(int x, int y) {
		return (equation.isHorizontal() && y == this.p1.getY())
				|| (equation.isVertical() && x == this.p1.getX())
	...
	public Line scale(double scale) {
		int newY = (int) (equation.isHorizontal() ? p1.getY() : equation
				.isVertical() ? p2.getY() * scale : equation.calculate(p2
				.getX() * scale));
	...
	public Line translate(int x, int y) {
		return new Line(p1.getX() + x, p1.getY() + y, p2.getX() + x, p2.getY()
				+ y);
	}
}
