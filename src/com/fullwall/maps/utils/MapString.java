...
public class MapString {
    ...
    public MapString colour(MapColour colour, int atIndex) {
        return create(string.substring(0, atIndex) + colour + string.substring(atIndex + 1, string.length()));
    }
    ...
    public MapString truncate(int targetWidth) {
        ...
        while (process.length() > 0 && font.getWidth(MapColour.strip(process)) > targetWidth) {
