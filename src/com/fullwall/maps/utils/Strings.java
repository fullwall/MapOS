...
public class Strings {
	...
	public static class Formatter {
		...
		public Formatter bracket() {
			return new Formatter(Strings.bracketize(format));
		}

		public Formatter capital() {
			return new Formatter(Strings.capitalise(format));
		}

		public Formatter colour() {
			return new Formatter(Strings.colourise(format));
		}

		public Formatter list() {
			return new Formatter(Strings.listify(format));
		}

		public Formatter plural(int amount) {
			return new Formatter(Strings.pluralise(format, amount));
		}
		...
		public Formatter wrap() {
			return new Formatter(Strings.wrap(format));
		}

		public Formatter wrap(ChatColor end) {
			return new Formatter(Strings.wrap(format, end));
		}
	}
	...
	public static String truncate(String string, int target) {
		return truncate(string, target, true);
	}

	public static String truncate(String string, int target, boolean stripVowels) {
		if (stripVowels)
			string = string.replaceAll("[aeiouAEIOU]", "");
		while (string.length() > 0 && string.length() > target) {
