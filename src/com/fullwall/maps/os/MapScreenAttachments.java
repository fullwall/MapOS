...
class MapScreenAttachments implements ScreenAttachments {
	...
	// TODO: remove this?
	private final SetMultimap<Key, KeyListener> keyListeners = HashMultimap
			.create();
	...
	private final Multiset<Class<? extends ScreenAttachment>> classes = HashMultiset
			.create();
	...
	public boolean contains(ScreenAttachment attachment) {
		if (attachment instanceof KeyListener
				&& keyListeners.get(((KeyListener) attachment).listenFor())
						.contains(attachment))
			return true;
		if (attachment instanceof MouseListener
				&& mouseListeners.contains(attachment))
			return true;
		if (attachment instanceof Animation
				&& runningAnimations.contains(attachment))
			return true;
		return false;
	}
	...
	private void internalAttach(ScreenAttachment toAttach) {
		...
		if (toAttach instanceof KeyListener)
			keyListeners.put(((KeyListener) toAttach).listenFor(),
					(KeyListener) toAttach);
	...
	public void processKeyEvent(KeyEvent event) {
		Iterator<KeyListener> iter = Iterators.concat(
				keyListeners.get(event.getKey()).iterator(),
