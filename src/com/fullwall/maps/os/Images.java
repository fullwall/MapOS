...
public class Images {
	...
	public static void loadImage(byte[] image, File src) {
		try {
			...
			System.arraycopy(MapPalette.imageToBytes(img), 0, image, 0,
					image.length);
	...
	public static void loadImage(byte[] image, File src, IconScale scale) {
		...
		if (image.length != scale.getArrayLength()) {
			throw new IllegalStateException(src.getName()
					+ " had an illegal size (expected "
	...
	static {
		...
		loadImage(image, new File("plugins/MapPlugin/images/missing.png"),
				IconScale.Layout);
