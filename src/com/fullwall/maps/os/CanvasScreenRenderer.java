...
class CanvasScreenRenderer implements ScreenRenderer {
	...
	private final SetMultimap<RenderPriority, CanvasRenderer> renderers = HashMultimap
			.create();
	private final SetMultimap<CanvasRenderer, MapPixel> pixels = HashMultimap
			.create();
	private final Multiset<Class<? extends CanvasRenderer>> classes = HashMultiset
			.create();
	...
	public boolean contains(CanvasRenderer attachment) {
		...
		return this.renderers.containsEntry(attachment.getPriority(),
				attachment);
	}
	...
	private void directRender(Screen screen, CanvasRenderer renderer,
			MapCanvas canvas) {
		...
		for (int i = 0; i < canvasBuffer.length; ++i) {
			if (canvasBuffer[i] >= 0) {
				MapPixel pixel = new MapPixel(new Point(i % 128, i / 128),
						canvasBuffer[i]);
	...
	public void setScreenRaw(byte[] buffer) {
		if (buffer.length != this.buffer.buffer.length)
			throw new IllegalArgumentException("expected array length of "
					+ this.buffer.buffer.length + ", got " + buffer.length);
	...
	static {
		try {
			setBaseMethod = CraftMapCanvas.class.getDeclaredMethod("setBase",
					byte[].class);
			setBaseMethod.setAccessible(true);
			getBufferMethod = CraftMapCanvas.class.getDeclaredMethod(
					"getBuffer", (Class<?>[]) null);
