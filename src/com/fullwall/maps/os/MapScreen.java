...
public class MapScreen implements Screen {
	...
	public void forwardMouseEvent(MouseClick click, boolean pressed) {
		if (click == MouseClick.Right
				&& screenRenderer.contains(SpringboardRenderer.class)) {
			mouse.setDown(!mouse.isDown());
		}
		this.attachments.processMouseEvent(new MouseClickEvent(mouse, this,
				click, pressed));
	}
	...
	@Override
	public void load(NBTKey root, boolean empty) {
		if (empty) {
	...
	public void moveMouse(int horizontal, int vertical) {
		...
		vertical *= vertical < 0 ? VERTICAL_SENSITIVITY_UP
				: VERTICAL_SENSITIVITY_DOWN;
		mouse.setX((byte) (mouse.getX() + horizontal));
		mouse.setY((byte) (mouse.getY() + vertical));
		attachments.processMouseEvent(new MouseMoveEvent(this, mouse,
				horizontal, vertical));
	}
	...
	public class Mouse {
		private final MapCursor mouse = new MapCursor((byte) 0, (byte) 0,
				(byte) 14, MapCursor.Type.WHITE_POINTER.getValue(), true);
	...
	private static final double HORIZONTAL_SENSITIVITY = 1,
			VERTICAL_SENSITIVITY_UP = 1.4, VERTICAL_SENSITIVITY_DOWN = 3;
}
